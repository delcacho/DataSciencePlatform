...
class ScalaMonitor:
    ...
    def target_func(self, comm, msg):
        ...
        self.sendUiPort(spark)

    def sendUiPort(self, spark):
        uiport = spark.sparkContext.uiWebUrl.split(":")[-1]
        logger.info('Send UIPORT '+uiport)
        self.send({
            'msgtype': 'uiport',
            'msg': uiport
        })
...
def load_ipython_extension(ipython):
    ...
    logger.info('Going to send UIPORT')
...
def unload_ipython_extension(ipython):
    ...
    if spark is not None:
       spark.stop()
    sc = ipython.user_ns.get('sc')
    if sc is not None:
       sc.stop()
    pass

def generateUiPort():
    with portalocker.Lock('.uiports.lock', 'w', timeout=60) as lk:
       try:
          with open('.uiports.log','r') as f:
             lines = f.read().splitlines()
       except:
          lines = []
       port = 4040
       portset = set([int(line.strip()) for line in lines])
       while port in portset:
          port = random.randint(4040,7000)
       portset.add(port)
    
       with open('.uiports.log', 'a') as f:
          f.write("{}\n".format(port))
    return(port)
